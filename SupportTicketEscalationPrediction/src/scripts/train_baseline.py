...
class BaselineModelTrainer:
    ...
    def baseline_training_run(self):
        ...
        """
        # Use local SQLite tracking by default (recommended over file store in newer MLflow versions).
        mlflow_enabled = True
        try:
            ...
            logger.info("MLflow tracking URI set to SQLite store: %s", mlflow_tracking_uri)
        except Exception as mlflow_error:
            mlflow_enabled = False
            logger.warning(
                "MLflow setup failed. Continuing training without MLflow logging. Error: %s",
                mlflow_error,
            )
        ...
        for model_name, model in self.models.items():
            run_ctx = mlflow.start_run(run_name=model_name) if mlflow_enabled else nullcontext()
            with run_ctx:
                ...
                if mlflow_enabled:
                    try:
                        ...
                        logger.warning(
                            "MLflow logging failed for %s. Continuing without MLflow for remaining models. Error: %s",
                            model_name,
                            mlflow_log_error,
                        )
                        mlflow_enabled = False
